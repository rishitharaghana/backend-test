...
const insertCrmUser = async (req, res) => {
  upload.fields([
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Missing or undefined required user fields" });
    }
    ...
    if (!validUserTypes.includes(parseInt(user_type))) {
      return res
        .status(400)
        .json({ error: "Invalid user_type. Must be 1, 2, 3, 4, 5, 6, or 7" });
    }

    if (![0, 1, 2].includes(parseInt(status))) {
      return res
        .status(400)
        .json({ error: "Invalid status. Must be 0, 1, or 2" });
    }

    if (parseInt(status) === 2 && !feedback) {
      return res
        .status(400)
        .json({ error: "Feedback is required when status is Rejected (2)" });
    }

    if ([0, 1].includes(parseInt(status)) && feedback) {
      return res.status(400).json({
        error:
          "Feedback must be null when status is Pending (0) or Approved (1)",
      });
    }
    ...
    if (isNaN(parseInt(created_user_id))) {
      return res
        .status(400)
        .json({ error: "created_user_id must be a valid integer" });
    }
    ...
    if (ifsc_code) {
      ...
      if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(ifsc_code)) {
        return res.status(400).json({
          error:
            "Invalid ifsc_code format. Must be 11 characters (e.g., SBIN0001234)",
        });
      }
    }
    ...
    try {
      ...
      const creatorResult = await queryAsync(
        "SELECT user_type FROM crm_users WHERE id = ?",
        [parseInt(created_user_id)]
      );
      if (!creatorResult.length) {
        return res
          .status(400)
          .json({ error: "Invalid created_user_id: User does not exist" });
      }
      ...
      if (creatorUserType === 1) {
        ...
        if (![3, 4, 5, 6, 7].includes(parseInt(user_type))) {
          return res.status(400).json({
            error: "Builder can only create user types 3, 4, 5, 6, or 7",
          });
        }
      } else {
        return res
          .status(403)
          .json({ error: "Only Admin (1) or Builder (2) can create users" });
      }
 ...
 const baseUrl = `https://crmapi.mntechs.com/uploads/`;
      ...
      const company_logo = req.files?.["company_logo"]?.[0]?.path
        ? `${baseUrl}${path.basename(req.files["company_logo"][0].path)}`
        : null;
      ...
      res
        .status(500)
        .json({ error: "Failed to insert user: " + error.message });
    }
  });
};
...
const editCrmUser = async (req, res) => {
  upload.fields([{ name: "photo", maxCount: 1 }])(req, res, async (err) => {
    ...
    if (
      isNaN(parseInt(created_user_id)) ||
      isNaN(parseInt(created_user_type))
    ) {
      return res.status(400).json({
        error: "Missing or invalid  created_user_id, or created_user_type",
      });
    }

    try {
      const userResult = await queryAsync(
        "SELECT user_type, created_user_id FROM crm_users WHERE id = ?",
        [parseInt(id)]
      );
      ...
      const updaterResult = await queryAsync(
        "SELECT user_type FROM crm_users WHERE id = ?",
        [parseInt(created_user_id)]
      );
      ...
      if (updaterUserType === 1) {
        ...
        if (userTypeToEdit !== 2) {
          return res
            .status(403)
            .json({ error: "Admin can only edit Builder (user_type 2)" });
        }
        ...
        if (![3, 4, 5, 6, 7].includes(userTypeToEdit)) {
          return res.status(403).json({
            error:
              "Builder can only edit Channel Partner, Sales Manager, Telecallers, Marketing Agent, or Receptionists",
          });
        }
        ...
        if (userToEdit.created_user_id !== parseInt(created_user_id)) {
          return res
            .status(403)
            .json({ error: "Builder can only edit their own employees" });
        }
      } else {
        return res
          .status(403)
          .json({ error: "Only Admin (1) or Builder (2) can edit users" });
      }
      ...
      if (userTypeToEdit === 2) {
        ...
        if (
          ...
          return res
            .status(400)
            .json({ error: "Invalid fields provided for Builder" });
        }
        ...
        if (
          ...
          return res
            .status(400)
            .json({ error: "Restricted fields provided for this user type" });
        }
      }
      ...
      if (status !== undefined) {
        ...
        if (parseInt(status) === 2 && !feedback) {
          return res.status(400).json({
            error: "Feedback is required when status is Rejected (2)",
          });
        }
        if ([0, 1].includes(parseInt(status)) && feedback) {
          return res.status(400).json({
...
//  const baseUrl = `http://localhost:3002/uploads/`;
      ...
      const updateFields = {
        ...
        created_user_id: parseInt(created_user_id),
        created_user_type: parseInt(created_user_type),
      };
      ...
      await queryAsync("START TRANSACTION");
      await queryAsync(`UPDATE crm_users SET ${setClause} WHERE id = ?`, [
        ...values,
        parseInt(id),
      ]);
      await queryAsync("COMMIT");
...
const deleteCrmUser = async (req, res) => {
  ...
  try {
    ...
    const deleterResult = await queryAsync(
      "SELECT user_type FROM crm_users WHERE id = ?",
      [parseInt(created_user_id)]
    );
    ...
    if (deleterUserType === 1) {
      if (userTypeToDelete !== 2) {
        return res
          .status(403)
          .json({ error: "Admin can only delete Builder (user_type 2)" });
      }
    } else if (deleterUserType === 2) {
      if (![3, 4, 5, 6, 7].includes(userTypeToDelete)) {
        return res.status(403).json({
          error:
            "Builder can only delete Channel Partner, Sales Manager, Telecallers, Marketing Agent, or Receptionists",
        });
      }
      if (userToDelete.created_user_id !== parseInt(created_user_id)) {
        return res
          .status(403)
          .json({ error: "Builder can only delete their own employees" });
      }
    } else {
      return res
        .status(403)
        .json({ error: "Only Admin (1) or Builder (2) can delete users" });
    }

    // Check for dependent records in lead_updates
    const dependentRecords = await queryAsync(
      "SELECT COUNT(*) as count FROM lead_updates WHERE updated_by_emp_id = ?",
      [parseInt(id)]
    );
    if (dependentRecords[0].count > 0) {
      return res.status(400).json({
        error:
          "Cannot delete user: They have associated lead updates. Please reassign or delete those records first.",
      });
    }
...
const updateUserStatus = async (req, res) => {
  ...
  if (!user_id || !status || !updated_by_user_id) {
    return res
      .status(400)
      .json({ error: "user_id, status and updated_by_user_id are required" });
  }
  if (isNaN(parseInt(user_id)) || isNaN(parseInt(updated_by_user_id))) {
    return res
      .status(400)
      .json({ error: "user_id and updated_by_user_id must be valid integers" });
  }
  ...
  if (status == 2 && !feedback) {
    return res
      .status(400)
      .json({ error: "Feedback is required when status is Rejected " });
  }
  if (status == 1 && feedback) {
    return res
      .status(400)
      .json({ error: "Feedback must be null when status is Approved " });
  }
  try {
    const userResult = await queryAsync(
      "SELECT user_type FROM crm_users WHERE id = ?",
    ...
    const updaterResult = await queryAsync(
      "SELECT user_type FROM crm_users WHERE id = ?",
      [parseInt(updated_by_user_id)]
    );
    if (!updaterResult.length) {
      return res
        .status(400)
        .json({ error: "Invalid updated_by_user_id: User does not exist" });
    }
    const updaterUserType = updaterResult[0].user_type;
    if (updaterUserType === 1) {
      if (![1, 2, 3, 4, 5, 6, 7].includes(targetUserType)) {
        return res
          .status(400)
          .json({ error: "Invalid user_type for target user" });
      }
    } else if (updaterUserType === 2) {
      if (![3, 4, 5, 6, 7].includes(targetUserType)) {
        return res.status(403).json({
          error: "Builder can only update user types 3, 4, 5, 6, or 7",
        });
      }
    } else {
      return res
        .status(403)
        .json({ error: "Only Admin  or Builder  can update user status" });
    }
    const currentDate = moment().format("YYYY-MM-DD");
    const currentTime = moment().format("HH:mm:ss");
    await queryAsync("START TRANSACTION");
...
const getUserTypesByBuilder = async (req, res) => {
  let { admin_user_id, emp_user_id, emp_user_type,status } = req.query;
console.log(status)
  ...
  try {
    ...
    if (emp_user_id) {
      const empUserIdArray = emp_user_id
        .split(",")
...
const getUserTypesCount = async (req, res) => {
  ...
  try {
    ...
    if (!admin_user_type || isNaN(parseInt(admin_user_type))) {
      return res.status(400).json({
        error: "admin_user_type is required and must be a valid integer",
      });
    }
    if (!admin_user_id || isNaN(parseInt(admin_user_id))) {
      return res.status(400).json({
        error: "admin_user_id is required and must be a valid integer",
      });
    }
    ...
    if (
      ...
      result = [
        {
          user_type: "today_leads",
          count: leadsResult[0].today_leads_count || 0,
        },
        {
          user_type: "today_follow_ups",
          count: leadsResult[0].today_follow_ups_count || 0,
        },
        {
          user_type: "site_visit_done",
          count: leadsResult[0].site_visit_done_count || 0,
        },
      ...
      result.push({
        user_type: "projects",
        count: projectsResult[0].project_count,
      });
    ...
    // Case 2: admin_user_type = 2 with emp_id and emp_user_type
      ...
      if (parsedEmpUserType === 3) {
        result.push({
          user_type: "booked",
          count: leadsResult[0].booked_count || 0,
        });
      }
      result.push({
        user_type: "projects",
        count: projectsResult[0].project_count || 0,
      });
    } else {
      return res.status(400).json({
        error:
          "Invalid admin_user_type or missing emp_id and emp_user_type for non-builder",
      });
    }
    ...
    console.error("Error fetching user types count:", {
      error,
  ...
  try {
    ...
    if (!emp_id && !emp_user_type) {
      ...
      if (!emp_id || isNaN(parseInt(emp_id))) {
        return res
          .status(400)
          .json({ error: "emp_id is required and must be a valid integer" });
      }
      if (!emp_user_type || isNaN(parseInt(emp_user_type))) {
        return res.status(400).json({
          error: "emp_user_type is required and must be a valid integer",
        });
      }
    ...
    if (usersResult.length === 0) {
      return res.status(404).json({
        error:
          !emp_id && !emp_user_type
            ? "Builder not found"
            : "No employees found for the given admin and employee details",
      });
    }
