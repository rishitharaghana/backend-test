...
const insertLead = async (req, res) => {
  ...
  if (
    lead_source_id === 6 &&
    (!assigned_id || !assigned_name || !assigned_emp_number)
  ) {
    return res.status(400).json({
      status: "error",
      message:
  ...
  const parsedAssignedUserType = assigned_user_type
    ? parseInt(assigned_user_type, 10)
    : null;
  ...
  try {
    ...
    if (projectCheck.length === 0) {
      return res
        .status(400)
        .json({ status: "error", message: "Invalid interested_project_id" });
    }
    const sourceCheck = await queryAsync(
      "SELECT lead_source_id FROM lead_source WHERE lead_source_id = ?",
      [parsedLeadSourceId]
    );
    if (sourceCheck.length === 0) {
      return res
        .status(400)
        .json({ status: "error", message: "Invalid lead_source_id" });
    }
    if (parsedAssignedId) {
      const userCheck = await queryAsync(
        "SELECT id FROM crm_users WHERE id = ?",
        [parsedAssignedId]
      );
      if (userCheck.length === 0) {
        return res
          .status(400)
          .json({ status: "error", message: "Invalid assigned_id" });
      }
    }
    const leadAddedUserCheck = await queryAsync(
      "SELECT id FROM crm_users WHERE id = ?",
    ...
    if (leadAddedUserCheck.length === 0) {
      return res
        .status(400)
        .json({ status: "error", message: "Invalid lead_added_user_id" });
    }
    ...
    if (defaultStatusCheck.length === 0) {
      return res.status(500).json({
        status: "error",
        message: "Default status not found in lead_statuses",
      });
    }
  ...
  try {
    ...
    if (updateResult.affectedRows === 0) {
      ...
      return res.status(404).json({
        status: "error",
        message: "Lead not found or you are not authorized to update this lead",
      });
    }
...
const getLeadsByUser = async (req, res) => {
  ...
  if (status_id) {
    ...
    const statusCheck = await queryAsync(
      "SELECT status_id FROM lead_statuses WHERE status_id = ?",
      [parsedStatusId]
    );
    if (statusCheck.length === 0 && parsedStatusId !== 0) {
      return res
        .status(400)
        .json({ status: "error", message: "Invalid status_id" });
    }
  }
  try {
    let query = `
    ...
    if (results.length === 0) {
      return res.status(404).json({
        status: "error",
        message: "No leads found for the given user",
      });
    }
    const formattedResults = results.map((lead) => ({
      ...
      assigned_date: lead.assigned_date
        ? moment(lead.assigned_date).format("YYYY-MM-DD")
        : null,
    }));
...
const getLeadsChannelPartner = async (req, res) => {
  const {
    lead_added_user_type,
    lead_added_user_id,
    assigned_user_type,
  ...
  if (status_id) {
    ...
    const statusCheck = await queryAsync("SELECT status_id FROM lead_statuses");
...
const assignLeadToEmployee = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      status: "error",
      message:
        "lead_id, assigned_user_type, assigned_id, assigned_name, assigned_emp_number, assigned_priority, followup_feedback, and next_action ,lead_added_user_type, and lead_added_user_id are required",
    });
  }
  const parsedLeadId = parseInt(lead_id, 10);
  const parsedLeadAddedUserType = parseInt(lead_added_user_type, 10);
  ...
  if (
    ...
    isNaN(parsedLeadAddedUserType) ||
    isNaN(parsedLeadAddedUserId) ||
    (status_id && isNaN(parsedStatusId))
  ) {
    return res.status(400).json({
      status: "error",
      message:
        "lead_id, lead_added_user_type, and lead_added_user_id must be valid integers; status_id must be a valid integer if provided",
    });
  }
  if (status_id) {
    const statusCheck = await queryAsync(
  ...
  try {
    ...
    if (!status_id) {
      const defaultStatus = await queryAsync(
        "SELECT status_id FROM lead_statuses WHERE status_name = 'open'"
      );
      if (defaultStatus.length === 0) {
        await queryAsync("ROLLBACK");
        return res.status(400).json({
          status: "error",
          message: "Default 'open' status not found",
        });
      }
...
const updateLeadByEmployee = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      status: "error",
      message:
        "lead_id, updated_by_emp_type, updated_by_emp_id, updated_by_emp_name, updated_emp_phone, lead_added_user_type, and lead_added_user_id are required",
    });
  }
  const parsedLeadId = parseInt(lead_id, 10);
  const parsedStatusId = status_id ? parseInt(status_id, 10) : null;
  ...
  if (
    isNaN(parsedLeadId) ||
    (status_id && isNaN(parsedStatusId)) ||
    isNaN(parsedUpdatedByEmpType) ||
    isNaN(parsedUpdatedByEmpId) ||
    isNaN(parsedLeadAddedUserType) ||
    isNaN(parsedLeadAddedUserId)
  ) {
    return res.status(400).json({
      status: "error",
      message:
        "lead_id, updated_by_emp_type, updated_by_emp_id, lead_added_user_type, and lead_added_user_id must be valid integers; status_id must be a valid integer if provided",
    });
  }
  if (status_id) {
    const statusCheck = await queryAsync(
  ...
  try {
    ...
    if (status_id) {
      const updateLeadsQuery = `
        ...
        WHERE lead_id = ?
      `;
      await queryAsync(updateLeadsQuery, [
        update_date,
        update_time,
        parsedStatusId,
        parsedLeadId,
...
const getLeadUpdatesByLeadId = async (req, res) => {
  ...
  if (!lead_id || !lead_added_user_type || !lead_added_user_id) {
    return res.status(400).json({
      status: "error",
      message:
        "lead_id, lead_added_user_type, and lead_added_user_id are required",
    });
  }
  const parsedLeadId = parseInt(lead_id, 10);
  const parsedLeadAddedUserType = parseInt(lead_added_user_type, 10);
  ...
  if (
    ...
    return res.status(400).json({
      status: "error",
      message:
        "lead_id, lead_added_user_type, and lead_added_user_id must be valid integers",
    });
  }
  try {
    const query = `
    ...
    if (results.length === 0) {
      return res.status(404).json({
        message:
          "No updates found for the given lead_id, lead_added_user_type, and lead_added_user_id",
      });
    }
...
const getBookedLeads = async (req, res) => {
  ...
  if (
    (assigned_user_type && !assigned_id) ||
    (!assigned_user_type && assigned_id)
  ) {
    return res.status(400).json({
      status: "error",
      message:
        "Both assigned_user_type and assigned_id must be provided together",
    });
  }
  ...
  try {
    ...
    const formattedResults = results.map((lead) => ({
      ...
      created_date: lead.created_date
        ? moment(lead.created_date).format("YYYY-MM-DD")
        : null,
      updated_date: lead.updated_date
        ? moment(lead.updated_date).format("YYYY-MM-DD")
        : null,
      assigned_date: lead.assigned_date
        ? moment(lead.assigned_date).format("YYYY-MM-DD")
        : null,
    ...
    console.error("Error fetching booked leads:", {
      error,
